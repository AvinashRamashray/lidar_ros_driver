...
class LIDAR_PRE_DLL CLidarPre {
 ...
 void findCameraUtc(FILE *lidar_fil , FILE *camera_fil,FILE *fil_out) ;
  ...
  int setAnlignPara(const char *cfgfile) ;  //配置文件配置外参
  int setCmdPara(PRE_IMU_S preimu, double centralMeridian);
  ...
  bool GeoModuelSelcet(DEVICE_TYPE_E mt_deviceTyp)  ; // 大地坐标解算—四维图新
...
private: 
 ...
 // PosEvent_Camera_S m_posEvent_Camera;
  ...
  bool m_trackfileisopen;
  char taildata[256];
  unsigned int m_devid;
  std::string m_clientname;
  APPROVE m_approve;
  int lascount;
  DEVICE_TYPE_E m_curdevtype;
...
//插入数据后，更新posEvent和Camera数据
  ...
  void resetCheckUSC();
  bool getTailData(char outdata[], char* filename, unsigned int& devid, int len);
public:

  DEVICE_TYPE_E getCurDevType(){return m_curdevtype;}
  APPROVE getApproveState();
  void setSlowFileOpened(bool isopened);
  int isFrom_ImpHead(bool imphead,PRE_CONTROL_S * pctrl);
  void setOutPutLasFmt();
  void setClientName(std::string clientname);
  void setOutPutLasCfg(int echo[]);
  void choiseModule(std::list<int>& tmp_proModule, DEVICE_TYPE_E& mt_deviceTyp);
  int getFileCount();
};
#endif
